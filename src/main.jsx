...
const additionalRepos = [
  { title: 'Regulatory Compliance Copilot', repo: 'https://github.com/bennyhin777-ui/Regulatory-Compliance-Copilot', tech: 'React · TypeScript · Tailwind · Supabase', description: 'An AI-assisted compliance copilot that helps teams interpret regulatory requirements, map controls to obligations, and surface gaps before audits. Built with Supabase for persistent state and a TypeScript + Tailwind front end.' },
];

const experience = [
  { role: 'Strategy & Analytics Business Analyst', company: 'Stripe', dates: 'New York, NY | Sep 2024 — Present', bullets: ['Analyze complex financial datasets using SQL and Python to identify revenue opportunities and optimize payment-routing strategies.', 'Develop Tableau and Looker dashboards to track KPIs and monitor product performance across multiple regions.', 'Build predictive models and AI-driven forecasts to anticipate transaction volumes and identify emerging market trends.', 'Design and execute A/B tests and statistical analyses to validate product strategies and inform data-driven decision making.', 'Collaborate with engineering teams in Agile and Scrum environments to streamline data pipelines and eliminate manual reporting processes.', 'Present findings and strategic recommendations to senior leadership with clear, actionable insights.'] },
  { role: 'CAP Analyst — Concession Abuse Prevention', company: 'Amazon', dates: 'Hyderabad, India | May 2023 — Oct 2023', bullets: ['Query large-scale transactional datasets using SQL and Excel to detect fraud patterns and identify unauthorized-access behaviors.', 'Build Tableau and Power BI reports to proactively monitor security risks and support threat-detection operations.', 'Manage high-volume case queues and escalate security tickets through Jira and Zendesk, ensuring timely resolution.', 'Apply root-cause analysis and risk modeling techniques to reduce exposure to account takeovers and social-engineering attacks.', 'Partner with cross-functional teams to enhance automated fraud-detection workflows and improve analyst efficiency.', 'Maintain process maps, technical documentation, and compliance audit materials to ensure regulatory readiness.'] },
  { role: 'Payment Risk Analyst', company: 'Coinbase', dates: 'Hyderabad, India | Sep 2020 — Apr 2023', bullets: ['Analyze fraud risk across ACH, card, and real-time payment rails using SQL and transaction-monitoring systems.', 'Design and refine risk controls, transitioning teams from heuristic-based rules to model-driven mitigation strategies.', 'Investigate fraud vectors using identity-provider data, Python scripts, and blockchain analytics to understand attacker methodologies.', 'Measure risk-strategy performance through precision, recall, Looker, and Tableau reporting.', 'Collaborate with Product and Engineering teams to strengthen fraud prevention while preserving seamless customer experiences.'] },
];
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        ...
        <a className="outline-button" href="/Bennyhin_Bapanapalli_Resume_(2).pdf" download>Download resume <span>↗</span></a>
      </header>
      ...
      <main id="top">
        <section className="hero page-width">
          <div className="hero-copy">
            ...
            <div className="proof-row"><div><strong>4+</strong><span>years in risk & analytics</span></div><div><strong>3</strong><span>high-volume payment environments</span></div><div><strong>5</strong><span>open-source repos on GitHub</span></div></div>
          </div>
        ...
        <section className="projects-section page-width" id="projects"><div className="section-heading"><div><p className="kicker">Portfolio projects</p><h2>Source code & project details.</h2></div><p className="section-note">All five public repositories from my GitHub account — four interactive project demos plus a regulatory compliance copilot.</p></div><div className="projects-list">{projects.map((project) => <a className="project-link-card" key={project.id} href={project.repo} target="_blank" rel="noreferrer"><div className="plc-header"><span className={`tab-dot ${project.color}`} /><strong>{project.title}</strong></div><p>{project.details}</p><div className="plc-footer"><span className="plc-tech">{project.tech}</span><span className="plc-link">GitHub <span>↗</span></span></div></a>)}</div><div className="additional-repos-heading"><p className="kicker">More on GitHub</p><h3>Additional repositories</h3></div><div className="projects-list">{additionalRepos.map((repo) => <a className="project-link-card" key={repo.repo} href={repo.repo} target="_blank" rel="noreferrer"><div className="plc-header"><span className="tab-dot blue" /><strong>{repo.title}</strong></div><p>{repo.description}</p><div className="plc-footer"><span className="plc-tech">{repo.tech}</span><span className="plc-link">GitHub <span>↗</span></span></div></a>)}</div><div className="portfolio-link-row"><span>Live portfolio:</span><a href={PORTFOLIO_URL} target="_blank" rel="noreferrer">{PORTFOLIO_URL} ↗</a></div></section>
      ...
      <footer className="footer page-width"><span>© 2026 Bennyhin Bapanapalli</span><span>Payments risk · AI business analysis · Operational intelligence</span><span><a href={PORTFOLIO_URL} target="_blank" rel="noreferrer">{PORTFOLIO_URL}</a> · <a href="/Bennyhin_Bapanapalli_Resume_(2).pdf" download>Download PDF ↗</a></span></footer>
